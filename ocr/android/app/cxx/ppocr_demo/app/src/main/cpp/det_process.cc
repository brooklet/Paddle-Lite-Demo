...
cv::Mat DetResizeImg(const cv::Mat img, int max_size_len,
    ...
    LOGI("DetResizeImg : %d, %d, %d, %d", img.cols, img.rows, max_size_len, ratio_hw.size());
    ...
    LOGI("ratio_hw[0]: %f, ratio_hw[1]: %f, resize_img.cols: %d, resize_img.rows: %d", ratio_hw[0],
         ratio_hw[1], resize_img.cols, resize_img.rows);
